...
本机仅支持 GGUF 量化模型（无 PyTorch），使用 `llama-omni-server` 作为推理后端。
...
| 进程 | 端口 | 说明 |
|------|------|------|
| llama-omni-server | 127.0.0.1:22500 | C++ 推理后端 |
| Worker | 0.0.0.0:22400 | Python 转发层 |
| Gateway | 0.0.0.0:8006 | WebSocket 入口 |

启动后浏览器访问: **http://localhost:8006**
...
/opt/llama.cpp-omni/build/bin/llama-omni-server \
  ...
  -ngl 99 --host 0.0.0.0 --port 22500
...
```bash
pkill -f "llama-omni-server\|worker.py\|gateway.py"
```

### 配置

模型路径和 backend 类型在 `config.json` 中配置：

```json
{
    "model": {
        "model_path": "/data/models/MiniCPM-o-4_5-gguf/MiniCPM-o-4_5-Q4_K_M.gguf"
    },
    "backend": "cpp"
}
```
